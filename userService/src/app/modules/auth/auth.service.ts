...
const verifyEmailToDB = async (payload: IVerifyEmail) => {
  ...
  const date = new Date();
  //  need to check from redis
  const redisData = await RedisService.getRedis(`user:${isExistUser._id}`);
  if (!redisData) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
  ...
  if (redisDataJson.oneTimeCode !== oneTimeCode) {
    throw new ApiError(StatusCodes.BAD_REQUEST, "You provided wrong otp");
  }
  if (date > redisDataJson.expireAt) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      "Otp already expired, Please try again",
    );
  ...
  let message;
  let data;
  await RedisService.deleteRedis(`user:${isExistUser._id}`);

  if (!isExistUser.verified) {
    await User.findOneAndUpdate({ _id: isExistUser._id }, { verified: true });
